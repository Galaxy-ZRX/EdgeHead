...
# modified by Ruixiao 2023/09/19
#####################
import math
...
def filter_same_pts_zrx(int_pts, num_of_inter):
    ...
    # print("filtered num: ", num_of_inter_new)
    return int_pts_new, num_of_inter_new

@cuda.jit('(float32[:], int32)', device=True, inline=True)
def area(int_pts, num_of_inter):
    int_pts_new, num_of_inter_new = filter_same_pts_zrx(int_pts, num_of_inter)
    area_val = 0.0
    for i in range(num_of_inter_new - 2):
        area_val += abs(
            trangle_area(int_pts_new[:2], int_pts_new[2 * i + 2:2 * i + 4],
                         int_pts_new[2 * i + 4:2 * i + 6]))
        # print("jmiddle step of area: ", area_val)
    return area_val
...
def line_segment_intersection(pts1, pts2, i, j, temp_pts):
    ...
    if acd != bcd:
        ...
        if abc != abd:
            ...
            temp_pts[0] = Dx / DH
            temp_pts[1] = Dy / DH
            # judge if temp_pts equal to one of ABCD - zrx
...
# # zrx ----------------------------------------------------
@cuda.jit('(float32, float32, float32[:])', device=True, inline=True, debug=True)
...
# zrx --------------------------------------------
@cuda.jit('(float32[:], float32[:])', device=True, inline=True, debug=True)
def inter_2(rbbox1, rbbox2):
    ...
    for i in range(4):
        sorted_indices[i] = i
    # Perform argsort based on the values in the local array
    for i in range(4):
        for j in range(i + 1, distance_list.shape[0]):
            if distance_list[i] > distance_list[j]:
                # Swap values in the local array
    ...
    #     index_list = argsort_local_array(distance_list, sorted_indices)
    max_index = sorted_indices[-1]
    #     second_index = sorted_indices[1]
    #     third_index = sorted_indices[2]
    corners1_new = cuda.local.array((6,), dtype=numba.float32)
    for i in range(3):
        if i < max_index:
            corners1_new[2 * i] = corners1[2 * i]
            corners1_new[2 * i + 1] = corners1[2 * i + 1]
        elif i >= max_index:
            corners1_new[2 * i] = corners1[2 * i + 2]
            corners1_new[2 * i + 1] = corners1[2 * i + 3]
    ...
    #     index_list = argsort_local_array(distance_list, sorted_indices)
    max_index_2 = sorted_indices_2[-1]
    #     second_index_2 = sorted_indices_2[1]
    #     third_index_2 = sorted_indices_2[2]
    corners2_new = cuda.local.array((6,), dtype=numba.float32)
    for i in range(3):
        if i < max_index_2:
            corners2_new[2 * i] = corners2[2 * i]
            corners2_new[2 * i + 1] = corners2[2 * i + 1]
        elif i >= max_index_2:
            corners2_new[2 * i] = corners2[2 * i + 2]
            corners2_new[2 * i + 1] = corners2[2 * i + 3]

    num_intersection = triangle_intersection(corners1_new, corners2_new,
                                             intersection_corners)
    #     assert num_intersection%2 == 3
    sort_vertex_in_convex_polygon(intersection_corners, num_intersection)
    # print(intersection_corners.reshape([-1, 2])[:num_intersection])
...
# zrx --------------------------------------------
@cuda.jit('(float32[:], float32[:], int32)', device=True, inline=True, debug=True)
def devRotateIoUEval_triangle_zrx(rbox1, rbox2, criterion=-1):
    area1 = rbox1[2] * rbox1[3] / 2
    area2 = rbox2[2] * rbox2[3] / 2
    area_inter = inter_2(rbox1, rbox2)
    if criterion == -1:
        #         return area_inter
        return area_inter / (area1 + area2 - area_inter)
    elif criterion == 0:
...
def rotate_iou_kernel_eval(N, K, dev_boxes, dev_query_boxes, dev_iou, criterion=-1):
    ...
    if tx < row_size:
        for i in range(col_size):
            ...
            dev_iou[offset] = devRotateIoUEval_triangle_zrx(block_qboxes[i * 5:i * 5 + 5],
                                           block_boxes[tx * 5:tx * 5 + 5], criterion)


def rotate_iou_gpu_eval_triangle(boxes, query_boxes, criterion=-1, device_id=0):
